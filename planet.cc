...
char *body[] = {"mercury", "venus", "earth", "moon", "mars",
                          "jupiter", "io", "europa", "ganymede", "callisto",
                          "saturn", "titan", "uranus", "neptune",
                          "pluto"};
                                                                              

planet::planet (double T)
{
  ...
  switch (opts.body)
    ...
    case NEPTUNE:
      {
	mlong = (poly (8.457994E1, 2.19885914E2, 3.205E-4, 0, T) * deg_to_rad);
	axis = 30.10957;
	ecc = poly (8.99704E-3, 6.33E-7, -2E-9, 0, T);
	incl = (poly (1.779242, -9.5436E-3, -9.1E-6, 0, T) * deg_to_rad);
	perih = (poly (2.76045975E2, 3.256394E-1, 1.4095E-4, 4.113E-6, T) * deg_to_rad);
	ascnode = (poly (1.30681389E2, 1.098935, 2.4987E-4, -4.718E-6, T) * deg_to_rad);
...
void 
planet::calc_geo (const double sunlon, const double sundist)
{
...
coordinates
planet::calc_coordinates (coordinates coords, double eps)
{
  compute_ra_dec (coords.lon, coords.lat, coords.alpha, coords.delta, eps);
  /*
  cout << "inclination = " << incl / deg_to_rad << '\n' 
       << "longitude of ascending node = " << ascnode / deg_to_rad << '\n' 
       << "longitude of perihelion = " << perih / deg_to_rad << '\n' 
       << "mean distance = " << axis << '\n' 
       << "eccentricity = " << ecc << '\n' 
       << "mean longitude = " << mlong / deg_to_rad << endl;
  
  cout << "heliocentric longitude = " << helio.lon / deg_to_rad << ", "
       << "heliocentric latitude = " << helio.lat / deg_to_rad << '\n'
       << helio.dist << endl;
  
  cout << "RA = " << coords.alpha << ", " 
       << "dec = " << coords.delta / deg_to_rad << endl;
  cout << "North pole alpha = " << alpha0/deg_to_rad << ", delta = " 
       << delta0/deg_to_rad << endl;
  */
  coords.alpha = TWO_PI * coords.alpha / 24.;

  axis_position = atan2 (cos (delta0) * sin (alpha0 - coords.alpha),
			 sin (delta0) * cos (coords.delta) 
			 - (cos (delta0) * sin (coords.delta) 
			    * cos (alpha0 - coords.alpha)));
  double lat = -asin (sin (delta0) * sin (coords.delta) 
		      + (cos (delta0) * cos (coords.delta) 
			 * cos (alpha0 - coords.alpha)));
  double lon = atan2 (sin (delta0) * cos (coords.delta) 
		      * cos (alpha0 - coords.alpha)
		      - cos (delta0) * sin (coords.delta),
		      cos (coords.delta) * sin (alpha0 - coords.alpha));
